	...
	public function capture_checklists($id)
	{
		...
		if($this->request->getMethod() === 'post'){
			if (!$this->validate('checklists')){
				...
				$userChecklists = $this->checklistsModel->getLatestChecklistUser($_SESSION['uid']);
				if(empty($userChecklists)){
					...
					$_SESSION['error_form'] = 'You already taken Health Declaration Form for your reason.';
					$this->session->markAsFlashdata('error_form');
					return redirect()->to( base_url('health%20declaration/captures/'.$_SESSION['uid']));
				}
			}
		}else{
			$data['viewName'] = 'Modules\HealthDeclaration\Views\healthdeclaration\frmChecklist';
